...
/**
 * Converts a plain event receiver registration function to [[AfterEvent]] keeper.
 *
 ...
 * @param initial  An event tuple to keep initially, or a function creating one. When omitted, the initial
  ...
  const afterEvent = ((receiver: EventReceiver<E>) => {
    ...
    if (!supply.isOff) {
      receiver.apply(
          ...
          lastEvent || (lastEvent = typeof initial === 'function' ? initial() : initial),
      );
      dest = receiver;
    }
 ...
 *
 * The event generated by `fallback` will be sent to new receivers unless `register` function sends one.
 *
 * @category Core
 * @typeparam E  An event type. This is a list of event receiver parameter types.
 * @param register  An event receiver registration function returning an event supply.
 ...
 * @param initial  A an event tuple to keep initially, or a function creating one.
 *
...
export function afterEventFrom<E extends any[]>(
  ...
  if (!isEventKeeper(supplier)) {
    return afterEventBy(supplier[OnEvent__symbol].bind(supplier), initial);
  }
